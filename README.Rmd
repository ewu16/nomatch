...
their related effect measures such as risk ratios, risk differences, and  vaccine effectiveness.

We use a simple simulated dataset mimicking data from an observational vaccine study,
although the methods are agnostic to the disease setting. 
...
- right censored survival data `(Y, event)` 
   - `Y` represents follow-up time for an outcome such as infection, hospitalization or death (`Y`)
   - `event` indicates whether individual experienced the event or censoring `1 = event, 0 = censored`
